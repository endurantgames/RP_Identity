...
function(self, event, ...)
    ...
    local function getRace(u)  local rpRace  = getText(u, "RA"); if mspRace  and mspRace:len()  > 0 then return mspRace  else return UnitRace(u)  end; end;
    local function getName(u)  local mspName  = getText(u, "NA"); if mspName  and mspName:len()  > 0 then return mspName  else return UnitName(u)  end; end;
    local function getClass(u) local mspClass = getText(u, "RC"); if mspClass and mspClass:len() > 0 then return mspClass else return UnitClass(u) end; end;

    local function getNameColor(u) return getColor(u, "NA") or "";           end;
     
    -- local mspClass = getText(u, "RC"); if mspClass then return mspClass; else local gameClass = UnitClass(u); return gameClass; end; end;
    -- local mspName = getText(u, "NA") if mspName then return mspName else local gameName = UnitName(u); return gameName; end;
    -- local mspRace = getField(u, "RA"); if mspRace then return mspRace; else local gameRace = UnitRace(u); return gameRace; end;

    local function getFirstName(u)
      local mspName = getText(u, "NA")
      if mspName then return mspName:gsub(" .+$", ""); else local gameName = UnitName(u) return gameName;
      end;
    end;
    ...
    local function getLastName(u)
      ...
      if mspName 
      then local parts = {};
           for part in mspName:gmatch("%S+") do table.insert(parts, part) end;
